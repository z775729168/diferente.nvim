...
local create_split = function(commit_win, ratio)
	...
	if width > vim.o.textwidth * 2 then
		M.resize = vim.o.textwidth
		split_func = "vsplit"
		resize_func = "vertical resize " .. vim.o.textwidth
	else
		M.resize = math.floor(ratio * height)
		split_func = "split"
...
local get_diff = function()
	...
	vim.api.nvim_command("silent r!git diff -u --cached --no-color --no-ext-diff")
	pcall(vim.api.nvim_command, [[silent! g/^  (use "git.*/d]])
	pcall(vim.api.nvim_command, [[silent! g/^$/d]])
...
local get_status = function()
	...
	-- Add and clean-up information to buffer
	vim.api.nvim_command("silent r!git -c color.status=false status -b")
	vim.api.nvim_command([[silent! g/^  (use "git.*/d]])
...
local get_log = function()
	...
	-- Add and clean-up information to buffer
	vim.api.nvim_command("silent r!git log --oneline")
	vim.api.nvim_win_set_cursor(win, { 1, 0 })
	vim.api.nvim_del_current_line()
	vim.bo.modifiable = false
...
local Diferente = function(opts)
	...
	M.commit_buf = vim.api.nvim_get_current_buf()
